...
i will attempt to import and clean all the tables 
### PGDP
here I will be working with Provincial GDP 
...
mutate(pct_change_import = ((Import / lag(Import, order_by = ref_date)) * 100) - 100) %>% View()
  mutate(first_dif = (value - lag(value, order_by = ref_date))) %>% View()
    #rename(trade = value) %>%
clean_PGDP %>%
  select(ref_date,geo,north_american_industry_classification_system_naics,CPC) %>%
  left_join(clean_population) %>% left_join(clean_population) %>% head
  
#clean the table with removing the population table. also


```

### Trade monthly 
This is the YOY percentage change difference for the monthly trade by NAPC
I will create the annual series tomorrow.
simply sum it over the year. it seems to be what the chief economist did and also it makes sense since it is already seasonally adjusted
```{r Trade Monthly}
...
mutate(geo = as_factor(geo),
         ref_date = lubridate::make_date(ref_date),
...
maybe use * in regex to match wild cards with the coordinates; for example 
1.1.***
