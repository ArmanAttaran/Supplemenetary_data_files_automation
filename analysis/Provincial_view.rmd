...
below is the raw trade data from NDM; it comes monthly and with a lot of data  I filter out most of it for this provincial view 
...
cpi %>%
  ...
  pivot_wider(names_from = products_and_product_groups, values_from = value) %>%
  select(!uom) -> clean_cpi
...
LFS_annual <- cansim::get_cansim_ndm(lfs_table_number)
...
LFS_annual %>%
  ...
  pivot_wider(names_from = labour_force_characteristics, values_from = value) %>% View()
  group_by(geo) %>%
  clean_names() -> clean_immigration
```
 

#### annual employment by indusury 
...
employment_by_industury %>%
    select(everything()) %>% 
  mutate(ref_date = parse_date(ref_date, "%Y")) %>%
  filter(ref_date > from_this_year) %>% View() 
...
created a schema for naming things; I should have followed the statcan names instead of naming things for simplicity. 
create code templates with text generations based on three scenarios 
annual tables
weekly tables
monthly tables 
