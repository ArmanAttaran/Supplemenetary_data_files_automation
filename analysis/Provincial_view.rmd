...
trade %>%
  ...
  mutate(pct_change_import = ((Import / lag(Import,n = 12,order_by = ref_date)) * 100) - 100) %>% View()
  #group_by(3,4,2) %>% 
  data.table::fwrite(here::here("delete", str_glue("Table_{trade_table_number}_Trade_full_table_from{from_this_year}")))
