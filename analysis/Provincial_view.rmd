...
all construction numbers are for total type of work and are in unadjusted K dollar 
...
data_tsbl %>%
  group_by_key() %>%
  index_by(year_month = ~ year(.)) %>%
  summarise(across(where(is.numeric), sum())) -> annual_construction
# saving the CSV {} between the
pivot_wider(names_from = investment_value, values_from = value) %>%
  head()
```
